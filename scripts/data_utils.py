...
def prepare_g2p_data(model_dir, train_path, valid_path, test_path):
    ...
    train_gr_length = sorted(max(train_gr_ids, key=len), reverse=True)[0]
    valid_gr_length = sorted(max(valid_gr_ids, key=len), reverse=True)[0]
    test_gr_length = sorted(max(test_gr_ids, key=len), reverse=True)[0]

    train_ph_length = sorted(max(train_ph_ids, key=len), reverse=True)[0]
    valid_ph_length = sorted(max(valid_ph_ids, key=len), reverse=True)[0]
    test_ph_length = sorted(max(test_ph_ids, key=len), reverse=True)[0]

    max_input_length = max(train_gr_length, valid_gr_length, test_gr_length)
    max_output_length = max(train_ph_length, valid_ph_length, test_ph_length)
    datasets = (train_gr_ids, train_ph_ids, valid_gr_ids, valid_ph_ids, test_gr_ids, test_ph_ids)
    vocabs = (gr_vocab, ph_vocab)
    params = (max_input_length, max_output_length, )

    return (train_gr_ids, train_ph_ids,
            valid_gr_ids, valid_ph_ids,
            gr_vocab, ph_vocab,
            test_dic, (max_input_length, max_output_length))
